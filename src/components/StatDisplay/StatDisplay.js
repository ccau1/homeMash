...
export default class StatDisplay extends Component {
  ...
  getTodayStart(): number {
    const {hour, minute} = this.props.settings.dayStart;
    return (new Date()).setHours(hour, minute, 0, 0);
  }

  getTodayEnd(): number {
    const {hour, minute} = this.props.settings.dayEnd;
    return (new Date()).setHours(hour, minute, 0, 0);
  }
  ...
  getMillisecondsWeekWorked(): number {
    ...
    let totalMs = 0;
    this.props.settings.daysOfWeek.forEach(dow => {
      if (dow < curDayOfWeek) {
        totalMs += totalDaily;
  ...
  getStatDisplay(): ReactElement<any> {
    const dayStart = this.getTodayStart();
    const dayEnd = this.getTodayEnd();
    const isWorkdayToday = this.isInDaysOfWeek(this.props.settings.daysOfWeek, this.getCurrentDayOfWeek());
    const totalDaily = dayEnd - dayStart;
    const totalWeekly = totalDaily * this.props.settings.daysOfWeek.length;
